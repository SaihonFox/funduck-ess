...
class DecisionTreeWidget(QWidget):
    def __init__(self, owner):
        ...
        pal = self.palette(); pal.setColor(self.backgroundRole(), QColor('white'))
    ...
    def doPaste(self):
        ...
        def updateNode(nd, data):
            ...
            for child_id in data.get('children', [])[:n_children]:
                t = gstate.addNewObject(ESNode()); nd.children.append(t); updateNode(t, data_dict[child_id])
            while len(nd.children) < n_children: nd.children.append(gstate.addNewObject(ESNode()))
        gstate.beginTransaction('Paste')
        try:
            self.clearNode(node); updateNode(node, gstate.clipboard[0])
        finally: gstate.endTransaction()

    def editCurrent(self, node):
        if node.content.getType() == kFactor: FactorDialog(node.content).exec_()
        elif node.content.getType() == kGoal: GoalDialog(node.content).exec_()

    def mousePressEvent(self, ev):
        if ev.button() == Qt.MiddleButton:
            self.panning = True; self.last_pos = ev.globalPos(); return
        node = self.getNodeUnderCursor(ev)
        if not node: return
        def set_sel(x): x.selected = (x == node)
        gstate.getRoot().traverse(set_sel); self.update()
    ...
    def mouseReleaseEvent(self, ev): self.panning = False

    def clearNode(self, node):
        gstate.modifyObject(node); node.content = None
    ...
    def setFactor(self, node):
        ...
        try:
            self.clearNode(node); node.content = factor
    ...
    def setGoal(self, node):
        ...
        try: self.clearNode(node); node.content = goal
        finally: gstate.endTransaction()
    ...
    def addExtraGoal(self, node):
        ...
        try: gstate.modifyObject(node); node.children.append(gstate.addNewObject(ESNode(None, goal)))
        finally: gstate.endTransaction()
    ...
    def keyPressEvent(self, ev):
        ...
        def getPrevNode(node):
            ...
            while True:
                ...
                if idx == 0: k += 1
                else: node = p.children[idx-1]; break
                node = p
            for _ in range(k):
        ...
        if key in [Qt.Key_Left, Qt.Key_Right, Qt.Key_Up, Qt.Key_Down]:
            tgt = gstate.getRoot() if not cur else (cur.parent if key == Qt.Key_Left else (cur.children[0] if cur.children and key == Qt.Key_Right else getPrevNode(cur) if key == Qt.Key_Up else getNextNode(cur)))
            if tgt: self.owner.selectNode(getId(tgt))
            return
        if not cur: return
        if int(ev.modifiers()) == 0:
            if key == Qt.Key_F: self.setFactor(cur)
            elif key == Qt.Key_G: self.setGoal(cur)
            elif key == Qt.Key_E: self.editCurrent(cur)
            elif key == Qt.Key_X: self.clearNodeUI(cur)
...
class MainW(QMainWindow):
    def __init__(self):
        ...
        self.act_export_rules = cmn.Action(self, 'Export the rules list...', '', self.doExportRules)
        ...
        for a in [self.act_new_es, self.act_open_es, self.act_save_es, self.act_save_es_as, None, self.act_export_png, self.act_export_rules, None, self.act_check_es, self.act_run_es, None, cmn.Action(self, 'Exit', '', self.exitApp)]:
        ...
        helpMenu = menubar.addMenu('Help'); helpMenu.addAction(self.act_about)
        toolbar = cmn.ToolBar([self.act_new_es, self.act_open_es, self.act_save_es, None, self.act_undo, self.act_redo, None, self.act_check_es, self.act_run_es, None, self.act_about])
        toolbar.setObjectName('tlb_main'); toolbar.setWindowTitle('Toolbar')
    ...
    def closingCheck(self):
        ...
        ans = QMessageBox.question(self, kProgramName, 'Save changes in expert system "%s"?' % gstate.getName(), QMessageBox.Yes | QMessageBox.No | QMessageBox.Cancel, QMessageBox.Yes)
    ...
    def updateUI(self):
        ...
        self.act_undo.setEnabled(gstate.canUndo()); self.act_redo.setEnabled(gstate.canRedo())
        gstate.getRoot().computeLayout(ESNode.kDiagramMargin, ESNode.kDiagramMargin)
        w, h = gstate.getExtents()
        self.pbox.setMinimumSize(w, h); self.pbox.update()

    def resetUI(self): self.check_results.reset(); self.updateUI(); self.pbox.setFocus()

    def doNew(self):
        if not self.closingCheck(): return
        gstate.resetState(); self.resetUI()

    def doOpenRaw(self, fname):
        try: gstate.loadFromFile(fname)
        except: QMessageBox.critical(self, kProgramName, 'An error occurred while opening file "%s"' % fname); gstate.resetState(); raise
        self.resetUI()
    ...
    def doSaveRaw(self, fname):
        try: gstate.saveToFile(fname)
        except: QMessageBox.critical(self, kProgramName, 'An error occurred while writing to file "%s"' % fname); raise
    ...
    def doExportPNG(self):
        ...
        if not fname: return
        w, h = gstate.getExtents()
        img = QImage(w, h, QImage.Format_RGB32); img.fill(QColor("white"))
        with QPainter(img) as p:
            p.setFont(ESNode.font); gstate.getRoot().render(p, True)
        img.save(fname, 'PNG')

    def doExportRules(self):
        if self.check_results.doCheck() >= kCheckError:
            QMessageBox.warning(self, kProgramName, 'There are errors in the expert system. Fix the errors and try again.'); return
        rules = []
        def gen(node, conds, ex_goals=[]):
            if not node.children:
                conds = '\n\tAND '.join(f'"{c[0]}" = "{c[1]}"' for c in conds) if conds else 'True'
                result = ', '.join(ex_goals + [node.content.name])
                rules.append('IF %s\nTHEN %s;' % (conds, result)); return
            if node.content.getType() == kFactor:
                for child, choice in zip(node.children, node.content.choices): gen(child, conds + [(node.content.name, choice)])
            else: gen(node.children[0], conds, ex_goals + [node.content.name])
        gen(gstate.getRoot(), [])
        cmn.showReport('Rules', 'Number of rules: %d\n' % len(rules) + '\n'.join(rules))

    def doCheckES(self): self.check_results.doCheck()
    def doEditDescription(self): DescriptionDialog().exec_()
    def doRunES(self):
        if self.check_results.doCheck() >= kCheckError:
            QMessageBox.warning(self, kProgramName, 'There are errors in the expert system.'); return
        ESWindow().exec_()
    def doGoals(self): GoalsDialog().exec_()
    def doFactors(self): FactorsDialog().exec_()
