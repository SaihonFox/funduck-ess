...
class ESWindow(cmn.Dialog):
    def __init__(self):
        ...
        self.setDialogLayout(layout, lambda: None, True, True, 
                             extra_buttons=[('Restart (F2)', self.doRestart)], autodefault=False)
    ...
    def updateUI(self):
        ...
        msg = 'Session is over'
        if cur is None:
            text = '<span style="color:red;">%s</span>' % 'Incomplete node reached'
            choices = []
        elif cur.getType() == kFactor:
            text = cur.name
            choices = cur.choices
            msg = 'Use digit buttons on the keyboard to choose answers quickly'
        else:
            assert(cur.getType() == kGoal)
            text = '<span style="color:green;">%s</span>' % 'Results'
            v = self.cur_node
            while True:
    ...
    def keyPressEvent(self, event):
        if event.isAutoRepeat():
            return
        if self.pending_release_idx is not None:
            self.updateUI()
            self.pending_release_idx = None
            return
        idx = cmn.eventToNum(event)
        if idx is not None and idx < len(self.cur_node.children):
